...
fname    <- 'set2_crc_labelled.csv'  #'A_reports-labelled-tnm_2023-03-03_131307.csv' # Name of file to be read
outname  <- 'set2_crc_labelled'  # First part of the name of file to be saved (date and .csv added later)
  ...
  sidebarLayout(
    sidebarPanel(
      fluidRow(#column(3, selectInput("prev", "Has previous staging?", choices = c(' ', 'yes', 'no'), selected=' ')),
               column(4, selectInput("red", "Has bad redaction?", choices = c(' ', 'yes', 'no'), selected=' ')),
               column(4, selectInput("crc_nlp", "Colorectal sample?", choices = c(' ', 1, 0), selected=' ')),
               column(4, textInput("note", "Comments", value = ' ', width = NULL, placeholder = NULL)),
               ),
      #h4('Action'),
